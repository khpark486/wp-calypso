	...
	describe( '#get', () => {
		...
		it( 'should handle get success', () => {
			...
			return handleRequest( store, action ).then( () => {
				...
				expect( store.dispatch ).to.have.been.calledWith(
					match( { type: WOOCOMMERCE_REQUEST_SUCCESS, action, data: getResponse } )
				);
				expect( store.dispatch ).to.have.been.calledWith(
		...
		it( 'should handle get failure', () => {
			...
			return handleRequest( store, action ).then( () => {
				...
				expect( store.dispatch ).to.have.been.calledWith(
					match( { type: WOOCOMMERCE_REQUEST_FAILURE, action: action } )
						.and( match.has( 'error' ) )
				);
				expect( store.dispatch ).to.have.been.calledWith(
	...
	describe( '#post', () => {
		...
		it( 'should handle post success', () => {
			...
			return handleRequest( store, action ).then( () => {
				expect( store.dispatch ).to.have.been.calledTwice;
				expect( store.dispatch ).to.have.been.calledWith(
					match( { type: WOOCOMMERCE_REQUEST_SUCCESS, action, data: postResponse } )
				);
				expect( store.dispatch ).to.have.been.calledWith(
					match( { type: onSuccessAction.type } )
						.and( match.has( 'data' ) )
	...
	describe( '#put', () => {
		...
		it( 'should handle put success', () => {
			...
			return handleRequest( store, action ).then( () => {
				expect( store.dispatch ).to.have.been.calledTwice;
				expect( store.dispatch ).to.have.been.calledWith(
					match( { type: WOOCOMMERCE_REQUEST_SUCCESS, action, data: putResponse } )
				);
				expect( store.dispatch ).to.have.been.calledWith(
					match( { type: onSuccessAction.type } )
						.and( match.has( 'data' ) )
	...
	describe( '#delete', () => {
		...
		it( 'should handle delete success', () => {
			...
			return handleRequest( store, action ).then( () => {
				expect( store.dispatch ).to.have.been.calledTwice;
				expect( store.dispatch ).to.have.been.calledWith(
					match( { type: WOOCOMMERCE_REQUEST_SUCCESS, action, data: deleteResponse } )
				);
				expect( store.dispatch ).to.have.been.calledWith(
					match( { type: onSuccessAction.type } )
						.and( match.has( 'data' ) )
